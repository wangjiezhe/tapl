...
let rec eval1 ctx t = match t with
  ...
  | TmProj(fi, (TmRecord(_, fields) as v1), l) when isval ctx v1 ->
      (try let (vari,ti) = List.assoc l fields
           in ti
...
let rec typeof ctx t =
  ...
  | TmProj(fi, t1, l) ->
      (match lcst ctx (typeof ctx t1) with
          TyRecord(fieldtys) ->
            (try let (varTi,tyTi) = List.assoc l fieldtys
               in tyTi
      ...
      (match lcst ctx tyT1 with
          TyRecord(fieldtys) ->
            (try
               ...
               if subtype ctx tyT2 tyTi then
                 tyT1
               else
                 error fi "type of new field value doesn't match";
             with Not_found -> error fi ("label "^l^" not found"))
        | _ -> error fi "Expected record type")
